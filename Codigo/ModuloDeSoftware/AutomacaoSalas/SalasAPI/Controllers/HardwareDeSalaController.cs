...
namespace SalasAPI.Controllers
{
    ...
    public class HardwareDeSalaController : ControllerBase
    {
        ...
        // POST: api/Hardware
        [HttpPost]
        public ActionResult Post([FromBody] HardwareDeSalaModel hardwareModel, uint idUser)
        {
        ...
        // PUT: api/Hardware/5
        [HttpPut("{id}")]
        public ActionResult Put(int id, [FromBody] HardwareDeSalaModel hardwareModel, uint idUser)
        {
            try
            {
                if (string.IsNullOrEmpty(hardwareModel.Ip) && hardwareModel.TipoHardwareId == TipoHardwareModel.CONTROLADOR_DE_SALA)
        ...
        [AllowAnonymous]
        public ActionResult GetSensorsAndActuatorsByIdSala([FromRoute] uint idSala, [FromQuery] string token)
        {
