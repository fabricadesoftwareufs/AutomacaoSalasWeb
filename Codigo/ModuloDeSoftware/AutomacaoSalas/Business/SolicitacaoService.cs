...
namespace Service
{
    public class SolicitacacaoService : ISolicitacaoService
    {
        ...
        public List<SolicitacaoModel> GetByIdHardware(uint idHardware, string tipo, bool todos = false)
        {
            var query = todos ?
                _context.Solicitacaos.Where(s => s.IdHardware == idHardware && s.TipoSolicitacao.Contains(tipo)) :
                _context.Solicitacaos.Where(s => s.IdHardware == idHardware && s.TipoSolicitacao.Contains(tipo) && !s.DataFinalizacao.HasValue);
        ...
        public bool Insert(SolicitacaoModel entity)
        {
            try
            {
                _context.Solicitacaos.Add(SetEntity(entity));
        ...
        public bool Remove(int id)
        {
            try
            {
                var solicitacao = _context.Solicitacaos.Where(s => s.Id == id);
                if (solicitacao != null)
                {
        ...
        public bool Update(SolicitacaoModel entity)
        {
            try
            {
                _context.Solicitacaos.Update(SetEntity(entity));
                return _context.SaveChanges() == 1;
            }
            catch (Exception)
            {
