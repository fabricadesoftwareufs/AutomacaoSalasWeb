...
namespace Service
{
    public class HardwareDeSalaService : IHardwareDeSalaService
    {
        ...
        public List<HardwareDeSalaModel> GetAtuadorByIdSala(int id)
            => _context.Hardwaredesala.Where(h => h.Sala == id && h.TipoHardware == (int)TIPO.MODULO_ATUADOR).Select(h => new HardwareDeSalaModel { Id = h.Id, MAC = h.Mac, SalaId = h.Sala, TipoHardwareId = h.TipoHardware, Ip = h.Ip, Token = h.Token, Uuid = h.Uuid }).ToList();

        public HardwareDeSalaModel GetControladorByIdSala(int idSala)
            => _context.Hardwaredesala.Where(h => h.Sala == idSala && h.TipoHardware == (int)TIPO.CONTROLADOR_SALA).Select(h => new HardwareDeSalaModel { Id = h.Id, MAC = h.Mac, SalaId = h.Sala, TipoHardwareId = h.TipoHardware, Ip = h.Ip, Token = h.Token, Uuid = h.Uuid }).FirstOrDefault();
